...
LOGS = logging.getLogger("KlllllIS")
...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jmthon.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/8ac0a640e41521b74ee89.jpg",
                caption="⌯︙**بــوت ميستري يـعـمـل بـنـجـاح**  ✅ \n᥀︙**قـنـاة الـسـورس**  :  @UX4SL\n᥀︙**مطور السورس**  :  @QVVV7\n᥀︙**مبرمج السورس**  :  @GZZZC\n᥀︙**مبرمج السورس**  :  @PCCC1",
                buttons=[(Button.url("قروب ميستري", "https://t.me/escanor_1bot"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**⌯︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت ميستري تمت بنجاح**"
            )
...
async def autojo():
    try:
        await jmthon(JoinChannelRequest("@UX4SL"))
        if gvar("AUTOEO") is False:
            ...
            try:
                ...
                pass
            try:
                await jmthon(JoinChannelRequest("@QVVV7"))
            except BaseException:
                pass
    except BaseException:
        pass
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @UX4SL"
        photobt = await jmthon.upload_file(file="Jmthon/razan/resources/start/Askanor.jpg")
        _, groupid = await create_supergroup(
            "مجموعة اشعارات اسكانور ", jmthon, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        descript = "⌯︙ وظيفه القروب يحفظ رسائل الخاص اذا ما تبي الامر احذف القروب نهائي \n  - @UX4SL"
        photobt = await jmthon.upload_file(file="Jmthon/razan/resources/start/Askanor.jpg")
        _, groupid = await create_supergroup(
            "مجموعة التخزين", jmthon, Config.TG_BOT_USERNAME, descript, photobt
        )
        addgvar("PM_LOGGER_GROUP_ID", groupid)
        print("تـم عمـل القروب التخزين بنـجاح واضافة الـفارات الـيه.")
        flag = True
    if flag:
